...
fn notary_authorization_assigns_network_id_correctly(svm: Rc<RefCell<LiteSVM>>) {
    let admin_client = initialize_and_create_admin_client(svm.clone());
    admin_client.create_network(0).expect("Failed to create network 0");
    admin_client.create_network(1).expect("Failed to create network 1");
    ...
    let authorization_notary_network_0_address = get_authorization_pda_for_address(&notary_network_0_keypair.pubkey());
    let authorization_notary_network_0 =
        get_pda::<NotaryAuthorization>(admin_client.svm.clone(), &authorization_notary_network_0_address)
            .unwrap()
            .unwrap();
...
fn state_processing_is_correct_across_networks(svm: Rc<RefCell<LiteSVM>>) {
    ...
    admin_client.create_network(network_0_id).expect("Failed to create network 0");
    let network_1_id: u16 = 1;
    admin_client.create_network(network_1_id).expect("Failed to create network 1");
    ...
    let notary_a_network_0_client =
        NotaryClient::new_with_keypair(svm.clone(), notary_a_network_0_key_pair.insecure_clone(), network_0_id);
    ...
    let notary_b_network_0_client =
        NotaryClient::new_with_keypair(svm.clone(), notary_b_network_0_key_pair.insecure_clone(), network_0_id);
    ...
    let notary_c_network_1_client =
        NotaryClient::new_with_keypair(svm.clone(), notary_c_network_1_key_pair.insecure_clone(), network_1_id);
    ...
    notary_a_network_0_client.commit(issue_tx_id, vec![]).unwrap();
    ...
    notary_a_network_0_client.commit(with_inputs_tx_id, vec![inputs_group(issue_tx_id, vec![1, 5])]).unwrap();
