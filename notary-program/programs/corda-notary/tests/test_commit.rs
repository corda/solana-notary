...
use crate::test_utils::test_helpers::{
    deploy_notary_program, generate_random_corda_tx_id, get_authorization_pda_for_address,
    input_index, inputs_group, ref_index, references_group,
};
...
fn notary_client() -> NotaryClient {
    ...
    admin_client
        .authorize_notary(&notary_key_pair.pubkey(), network_id)
        .expect("Failed to authorize notary");

    // insecure cloning should be okay for testing purposes
    let notary_client =
        NotaryClient::new_with_keypair(svm_rc, notary_key_pair.insecure_clone(), network_id);
...
fn commit_issuance(notary_client: NotaryClient) {
    ...
    assert!(
        !was_pre_commited.unwrap(),
        "Transaction should not be committed before"
    );
...
fn commit_creates_inputs_if_not_present(notary_client: NotaryClient) {
    ...
    assert_tx!(
        ...
        input_id,
        false,
        "Input should not be known  before"
    );

    notary_client
        .commit(tx_id, vec![inputs_group(input_id, vec![1, 5])])
        .unwrap();
...
fn cannot_double_spend_input(notary_client: NotaryClient) {
    ...
    assert_tx!(
        ...
        new_tx_id,
        false,
        "Double spend attempt should not create output"
    );
}
...
fn can_spend_other_output(notary_client: NotaryClient) {
    ...
    notary_client
        .commit(new_tx_id, vec![inputs_group(input_id, vec![2, 3])])
        .unwrap();
    ...
    assert_tx!(
        ...
        new_tx_id,
        true,
        "Output transaction should exist"
    );
}
...
fn can_spend_inputs_from_multiple_txs(notary_client: NotaryClient) {
    ...
    let inputs = vec![
        inputs_group(input_id1, vec![1, 5]),
        inputs_group(input_id2, vec![2]),
    ];
    ...
    assert_tx!(
        ...
        input_id1,
        true,
        "Input1 should be known after"
    );
    assert_tx!(
        notary_client,
        input_id2,
        true,
        "Input2 should be known after"
    );
}
...
fn can_spend_output_from_previous_output(notary_client: NotaryClient) {
    ...
    notary_client
        .commit(new_tx_id, vec![inputs_group(tx_id, vec![2, 3])])
        .unwrap();
...
fn commit_fails_when_input_accounts_are_missing(notary_client: NotaryClient) {
    ...
    let input_id = generate_random_corda_tx_id();
    let authorized_notary_pda =
        get_authorization_pda_for_address(&notary_client.get_client_pubkey());

    let accounts =
        notary_client.create_accounts_for_commit(authorized_notary_pda, Some(tx_id), vec![]);

    let result = notary_client.commit_with_custom_accounts(
        tx_id,
        vec![inputs_group(input_id, vec![0])],
        accounts,
    );

    assert_is_solana_error!(
        result,
        "Missing input account should fail",
        NotaryError::InvalidAccounts
    );
    assert_tx!(
        notary_client,
        tx_id,
        false,
        "Transaction should not be committed after fauilure"
    );
    assert_tx!(
        notary_client,
        input_id,
        false,
        "Input should not be present after fauilure"
    );
}
...
fn commit_fails_with_duplicate_input_accounts(notary_client: NotaryClient) {
    ...
    let accounts = notary_client.create_accounts_for_commit(
    ...
    assert_is_solana_error!(
        result,
        "Duplicate input account should lead to failure",
        NotaryError::InvalidAccounts
    );
    assert_tx!(
        notary_client,
...
fn commit_fails_when_output_account_is_missing(notary_client: NotaryClient) {
    ...
    let accounts =
        notary_client.create_accounts_for_commit(authorized_notary_pda, None, vec![input_id]);
    ...
    assert_is_solana_error!(
        result,
        "Missing output account should lead to failure",
        NotaryError::InvalidAccounts
    );
    assert_tx!(
        notary_client,
...
fn can_commit_without_consuming_reference_state(notary_client: NotaryClient) {
    ...
    assert_tx!(
        ...
        reference_id,
        false,
        "Reference state should not be known before"
    );
    ...
    assert_tx!(
        ...
        reference_id,
        true,
        "Reference state should be known here"
    );
...
fn cannot_commit_with_consumed_reference_state(notary_client: NotaryClient) {
    ...
    assert_is_solana_error!(
        result,
        "Consumed reference state should lead to failure",
    ...
    assert_tx!(
        ...
        new_tx_id,
        false,
        "Output transaction should not exist"
    );
}
...
fn can_use_inputs_and_reference_states_from_same_existing_tx(notary_client: NotaryClient) {
    ...
    let new_tx_id = generate_random_corda_tx_id();
    let test_input = vec![StateRefGroup::new(
        tx_id,
        vec![input_index(2), input_index(3), ref_index(1)],
    )]; // inputs and refs are different indices of same tx
...
fn cannot_use_same_state_as_input_and_reference_state_from_exising_tx(notary_client: NotaryClient) {
    ...
    let test_input = vec![StateRefGroup::new(
        tx_id,
        vec![input_index(1), ref_index(1)],
    )]; // inputs and refs are same index of same tx

    let result = notary_client.commit(new_tx_id, test_input);

    assert_is_solana_error!(
        result,
        "Cannot use same state as input and reference!",
        NotaryError::Conflict
    );
    assert_tx!(
        notary_client,
