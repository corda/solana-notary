...
impl NotaryClient {
    ...
    pub fn is_authorized_notary(&self, address: &Pubkey) -> Result<bool, anchor_client::ClientError> {
    ...
    pub fn get_corda_tx_account(&self, tx_id: TxId, network_id: u16) -> Pubkey {
        let (output_account, _) = Pubkey::find_program_address(
            &["corda_tx".as_bytes(), &tx_id.as_ref(), network_id.to_le_bytes().as_ref()],
            &corda_notary::ID,
        );
        output_account
    }

    pub fn commit(&self, tx_id: TxId, inputs_and_references: Vec<StateRefGroup>) -> TransactionResult {
        let authorized_notary_pda = get_authorization_pda_for_address(&self.get_client_pubkey());
        self.commit_with_custom_authorization_pda(tx_id, inputs_and_references, authorized_notary_pda)
    }
    ...
    pub fn create_accounts_for_commit(
        ...
        if output.is_some() {
            accounts.push(AccountMeta::new(self.get_corda_tx_account(output.unwrap(), self.network_id), false))
        }

        let mut remaining_accounts = inputs_and_references
            .iter()
            .map(|txhash| AccountMeta::new(self.get_corda_tx_account(*txhash, self.network_id), false))
            .collect::<Vec<_>>();
    ...
    pub fn commit_with_custom_authorization_pda(
        ...
        let input_ids = inputs_and_references.iter().map(|group| group.txhash).collect::<Vec<_>>();
    ...
    pub fn get_tx_account(&self, tx_id: TxId) -> Result<Option<CordaTxAccount>, anchor_client::ClientError> {
