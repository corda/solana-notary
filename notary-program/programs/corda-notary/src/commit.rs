...
pub fn do_commit<'a, 'b, 'c, 'info>(
    ...
    if ctx.remaining_accounts.len() != 1 /* output */ + inputs_and_references.len() {
        msg!("InvalidAccounts: expected {} remaining accounts", inputs_and_references.len() + 1);
    ...
    for (group_index, state_ref_group) in inputs_and_references.iter().enumerate() {
        ...
        let corda_tx_account_info =
            get_and_create_corda_tx_account(ctx, &state_ref_group.txhash, INPUT_OFFSET + group_index)?;

        let mut corda_tx_account = CordaTxAccount::try_deserialize(&mut &corda_tx_account_info.data.borrow()[..])?;

        for (group_indices_index, &flagged_state_ref_index) in state_ref_group.flagged_indices.iter().enumerate() {
...
fn get_and_create_corda_tx_account<'a, 'b, 'c, 'info>(
    ...
    if account_info.data_is_empty() {
        ...
        msg!("InvalidAccountOwner: {} account owner ({}) is not the program", index, account_info.owner);
...
fn get_corda_tx_account<'a, 'b, 'c, 'info>(
    ...
    let (expected_address, bump) = Pubkey::find_program_address(
        &[b"corda_tx", txhash.as_ref(), ctx.accounts.authorization.network_id.to_le_bytes().as_ref()],
        ctx.program_id,
    );
    ...
    if *account_info.key != expected_address {
        msg!("InvalidAccounts: {} expected {}, actual {}", index, expected_address, *account_info.key);
        return err!(NotaryError::InvalidAccounts);
    }
...
fn create_corda_tx_account<'a, 'b, 'c, 'info>(
    ...
    invoke_signed(
        ...
        &[ctx.accounts.notary.to_account_info(), account_info.clone(), ctx.accounts.system_program.to_account_info()],
        &[&[b"corda_tx", tx_id.as_ref(), ctx.accounts.authorization.network_id.to_le_bytes().as_ref(), &[bump]]],
    )?;
